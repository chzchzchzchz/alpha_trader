...
class Swarm:
    ...
    def predict(self, price):
        """Get consensus YES probability — with meaningful disagreement."""
        votes = []
        for a in self.agents:
            ...
            if p == 'quant':
                val = (1 - price) * 0.25 + random.gauss(0, 0.1) if ch else price * 0.7 + random.gauss(0, 0.15)
            elif p == 'contrarian':
                val = 0.22 + random.gauss(0, 0.15) if ch else (1 - price) * 0.8 + noise
            elif p == 'degen':
                val = 0.18 + b * 0.3 + random.gauss(0, 0.2) if ch else price * (1 + b * 0.5) + random.gauss(0, 0.15)
            elif p == 'news':
                val = 0.25 + random.gauss(0, 0.15) if ch else price + b * 0.1 + noise
            elif p == 'bull':
                val = price * (1 + b * 0.3) + random.gauss(0, 0.12) + 0.05
            elif p == 'bear':
                val = price * (1 - abs(b) * 0.2) - abs(b) * 0.12 + random.gauss(0, 0.1) - 0.03
            elif p == 'momentum':
                val = price + v * 0.08 + random.gauss(0, 0.1)
            else:
                val = price * 0.8 + 0.05 + noise
...
def analyze_market(m, swarm):
    ...
    # Strategy 1: Cheap YES (1-10c ask) — swarm says ANY positive edge
    if 1 <= ask_c <= 10:
        cost = ask_c
        if swarm_yes > (cost / 100.0):  # ANY positive edge on cheap contracts
    ...
    if bid_c >= 90:
        no_cost = 100 - bid_c
        if no_cost <= 10 and swarm_no > (no_cost / 100.0):
    ...
    # Strategy 3: Mid-price value (5-20c YES or 80c+ YES bid) — swarm disagrees 2%+
    if 5 <= ask_c <= 20:
        if swarm_yes > (ask_c / 100.0) + 0.02:
            cost = ask_c
            edge = swarm_yes - (cost / 100.0)
            exp_pnl = swarm_yes * (100 - cost) - (1 - swarm_yes) * cost - 2
            if exp_pnl > 2:
    ...
    if bid_c >= 80:
        no_cost = 100 - bid_c
        if no_cost <= 20 and swarm_no > (no_cost / 100.0) + 0.02:
            edge = swarm_no - (no_cost / 100.0)
            exp_pnl = swarm_no * (100 - no_cost) - (1 - swarm_no) * no_cost - 2
            if exp_pnl > 2:
    ...
    # Strategy 4: HIGH volume (500+) ANY positive EV
    if vol >= 500:
        if ask_c <= 30 and swarm_yes > (ask_c / 100.0) + 0.01:
            cost = ask_c
            exp_pnl = swarm_yes * (100 - cost) - (1 - swarm_yes) * cost - 2
            if exp_pnl > 2:
        ...
        no_cost = 100 - bid_c
        if no_cost <= 30 and swarm_no > (no_cost / 100.0) + 0.01:
            exp_pnl = swarm_no * (100 - no_cost) - (1 - swarm_no) * no_cost - 2
            if exp_pnl > 2:
